...
class AppService
{
    ...
    {
        try {
            $http = Http::baseUrl(config('app.service_api') . '/api/v1')->timeout(300)->withoutVerifying();

            $versions = $http->withHeaders([
                'X-License-Key' => $licenseKey,
                'X-Identifier' => $this->getDomain($domain),
            ])->get('/versions');

            if (is_null($versions->json())) {
                throw new ServiceException('授权服务器异常，请稍后再试');
            }
    ...
    public function getLskyProVersions(): array
    {
        try {
            ...
            $versions = $http->withHeaders([
                'X-License-Key' => $appSettings->license_key,
                'X-Identifier' => $this->getDomain($appSettings->url),
            ])->get('/versions');

            if (is_null($versions->json())) {
                throw new ServiceException('更新服务器异常，请稍后再试');
            }

            if (!$versions->ok()) {
                throw new ServiceException($versions->json('message'));
            }
        } catch (Throwable $e) {
            throw new ServiceException($e->getMessage());
        }
